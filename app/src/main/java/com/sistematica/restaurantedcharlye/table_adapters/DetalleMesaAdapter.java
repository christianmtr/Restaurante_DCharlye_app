package com.sistematica.restaurantedcharlye.table_adapters;
...
/**
 * Created by christianmtr on 22/02/17.
 */

public class DetalleMesaAdapter {
    // Variables de la clase

    private TableLayout tl_detalle_mesa; // Layout donde se pintará la tabla
     ...
     */
    public DetalleMesaAdapter(Activity actividad, TableLayout tabla) {
        this.actividad = actividad;
        this.tl_detalle_mesa = tabla;
...
//            texto.setLayoutParams(layoutCelda);
    ...
    public void agregarFilaTabla(ArrayList<String> elementos) {
        ...
        for (int i = 0; i < elementos.size(); i++) {
            ...
            texto.setGravity(Gravity.CENTER_HORIZONTAL);
            texto.setTextAppearance(actividad, R.style.tabla_celda);
...
//            texto.setLayoutParams(layoutCelda);
    ...
    public void eliminarFila(int indicefilaeliminar) {
        if (indicefilaeliminar > 0 && indicefilaeliminar < FILAS) {
            tl_detalle_mesa.removeViewAt(indicefilaeliminar);
            FILAS--;
        }
    }
